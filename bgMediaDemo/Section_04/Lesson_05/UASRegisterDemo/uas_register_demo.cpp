...
void sendRegisterAnswer( sipRegisterInfo&info)
{
    ...
    {
        int result = ::eXosip_message_build_answer(sip_context_, info.baseInfo.sipRequestId,
                iStatus, &answer);
        if (iStatus == 401)
        {
        ...
        {
            ::eXosip_message_send_answer(sip_context_, info.baseInfo.sipRequestId, 400, NULL);
        }
        else
        {
            //发送消息体
            ::eXosip_message_send_answer(sip_context_, info.baseInfo.sipRequestId, iStatus,
                    answer);
        }
        if (0 == info.baseInfo.expires)
        {
            eXosip_register_remove(sip_context_, info.baseInfo.sipRequestId);
        }
...
int main()
{
    ...
    if (OSIP_SUCCESS != eXosip_listen_addr(sip_context_, IPPROTO_UDP, NULL, UASPORT, AF_INET,
            0))
    {
    ...
    {
        // Wait the osip event.
        osipEventPtr = ::eXosip_event_wait(sip_context_, 0, 200);// 0的单位是秒，500是毫秒
